...
// CustomerSignIn 验证外卖用户登录
func (c *ControllerV1) CustomerSignIn(ctx context.Context, req *v1.CustomerSignInReq) (res *v1.CustomerSignInRes, err error) {
	var customer entity.Customer
	err = dao.Customer.Ctx(ctx).Where("name", req.Name).Where("password", req.Password).Scan(&customer)
	// if err != nil {
	// 	return nil, err
	// }
	// if customer.Uid == 0 {
	...
	return &v1.CustomerSignInRes{Uid: int64(customer.Uid)}, nil
}
